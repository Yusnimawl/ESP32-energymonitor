...
int lag,lead;
    ...
    boolean lastICross, checkICross;                  //Used to measure number of times threshold is crossed.
...
double Readpoly(byte pin){
  double reading = analogRead(pin); // Reference voltage is 3v3 so maximum reading is 3v3 = 4095 in range 0 to 4095
  if(reading < 1 || reading > 4095) return 0;
//  return -0.000000000009824 * pow(reading,3) + 0.000000016557283 * pow(reading,2) + 0.000854596860691 * reading + 0.065440348345433;
return  (-0.000000000000016 * pow(reading,4) + 0.000000000118171 * pow(reading,3)- 0.000000301211691 * pow(reading,2)+ 0.001109019271794 * reading + 0.034143524634089)/(3.3/4096)-41;
...
void calcVI(unsigned int crossings, unsigned int timeout){
  ...
  while(st==false)                                   //the while loop...
  {
    ...
    startV = Readpoly(inPinV);                    //using the voltage waveform
    //Serial.println(startV); //1950-1965
  
    if ((startV < (2030)) && (startV > (2070)))//check its within range
...
//    sampleOffset = analogRead(inPinOffset);                 //Read in raw offset
    ...
    sampleV = Readpoly(inPinV);                 //Read in raw voltage signal
    sampleI = Readpoly(inPinI);                 //Read in raw current signal
    sampleOffset = Readpoly(inPinOffset);                 //Read in raw offset
...
//offsetI = sampleOffset + ((sampleI+2-offsetI)/4096) ;    
//filteredV = (sampleV) - sampleOffset;
//filteredI = sampleI+2- sampleOffset;// + ((sampleI+2-offsetI)/4096);//-5.4;+0.36703 
//Serial.println(filteredI);
  ...
  //long hm = 0;
    ...
    if(filteredI>0)checkICross = true;
              else checkICross = false;    
    if (numberOfSamples==1) lastICross = checkICross;
    if (lastICross != checkICross) startfI++;;
    
    if(phaseShiftedV>0){vnow=1;}//v+
    if(phaseShiftedV<0){vnow=0;}//v-
    if(filteredI>0){inow=1;}//i+
    if(filteredI<0){inow=0;}//i-
    if(vlast==0&&vnow==1){
      if(inow==0){lag++;//LAG  
      } else {lead++;}    //LEAD
      }
...
if(lag<lead)laglead=0;
if(lag>lead)laglead=1;
  ...
  Serial.print("f V: ");
    ...
    Serial.println(crossCount); //f teg 
Serial.print("f I: ");
  Serial.println(startfI); //f arus
  double V_RATIO;
  double I_RATIO;
...
/* if(startfI>300&&realPower<=1){ //highly distorted
  Irms=realPower/Vrms;
if(startfI>600){
 Irms=0;
 realPower=0;
...
void serialprint(){ 
  ...
  Serial.println(" deg");
  Serial.print("harmonics order: ");
  Serial.println(harmonics);


  //if(lead==0&&lag==1&&powerFactor!=1){Serial.print(" lag");}
  //if(lead==1&&lag==0&&powerFactor!=1){Serial.print(" lead");}
if(laglead==1&&powerFactor<1){Serial.println("Inductive");}
if(laglead==0&&powerFactor<1){Serial.println("Capacitive");}
if(harmonics<4&&powerFactor>=0.99){Serial.println("Unity");}
if(harmonics!=1&&powerFactor<1){Serial.println("Distorted");}
...
int i=5;
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(),MQTT_USER,MQTT_PASSWORD)) {
      ...
      client.publish("/ic/presence/energy/", "Energy Monitor Activated");
      // ... and resubscribe
...
void setup() {
  ...
  offset(biasread);
  voltage(Tegangan, 341.6, 7);        // Voltage: input pin, calibration, phase_shift
  current(Arus, 12.1212 );       // Current: input pin, calibration.
...
//    if(harmonics>2){
//      publishll("Distorted, order 2+ harmonics");
//       client.publish(MQTT_C_PUBLISH_CH, String(CuF*(0)).c_str());
//      Serial.println("Distorted, order 2+ harmonics");}
//    
//      
//    
//    client.loop();   
